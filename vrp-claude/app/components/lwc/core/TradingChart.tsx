...
// Minimal base64 protobuf decoder for Yahoo Finance websocket
function decodeYFMessage(base64str: string) {
    ...
    while (i < buffer.length) {
        ...
        if (type === 0) { // varint
            ...
            if (field === 1) {
                const strBytes = buffer.slice(i, i + len);
                result.id = new TextDecoder().decode(strBytes);
            }
            i += len;
        } else if (type === 5) { // 32-bit float
            if (field === 2) {
                const view = new DataView(buffer.buffer, buffer.byteOffset + i, 4);
                result.price = view.getFloat32(0, true);
            } else if (field === 8) {
                const view = new DataView(buffer.buffer, buffer.byteOffset + i, 4);
                result.changePercent = view.getFloat32(0, true);
            }
            i += 4;
        } else {
            break; // Unknown type, abort safely
        }
    }
    return result;
}
...
export default function TradingChart({ 
    ...
    useEffect(() => {
            ...
            try {
                ...
                if (isMounted) {
                    ...
                    if (candleData.length > 0) {
                        lastCandleRef.current = { ...candleData[candleData.length - 1], volume: formattedData[formattedData.length - 1].volume };
                    }
                    if (volData.length > 0) {
        ...
        const initWebSocket = () => {
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    if (data && data.id === ticker && data.price && isMounted && lastCandleRef.current) {
                        const price = data.price;
                        const current = lastCandleRef.current;
                        
                        // Update current candle bounds seamlessly
                        current.close = price;
                        current.high = Math.max(current.high, price);
                        current.low = Math.min(current.low, price);
                        
                        if (candlestickSeriesRef.current) {
                            candlestickSeriesRef.current.update(current);
                        }
                        
                        if (lastVolumeRef.current && volumeSeriesRef.current) {
                            lastVolumeRef.current.color = current.close > current.open ? 'rgba(16, 185, 129, 0.4)' : 'rgba(239, 68, 68, 0.4)';
        ...
        // Kurangi interval REST polling menjadi 1 menit karena WS sudah menangani ticks real-time
